...
```typescript
// TypeScript — runtime errors, garbage-collection pauses
async function processData(data: any[]) {
  return data.map((item) => item.value * 2); // What if item.value is undefined?
}
```
...
fn process_data(data: &[Item]) -> Vec<i32> {
    ...
    // Compiler guarantees: no null, no data races, no memory leaks
}
```
...
8. **Real-World Example** — production-ready code
...
**Suggested paths:** 🏃 Quick (20–30h): 01, 02, 03, 04, **05**, 06, 08, 11, 12, 16 · 🚶 Standard (60–80h): 00–19, skim 20–26, review 30 · 🎓 Complete (120–150h): all 31 chapters + projects.
...
| **Null Safety**       | null/undefined everywhere     | `Option<T>`, no null                    |
| **Error Handling**    | try/catch, throw              | `Result<T, E>`, no exceptions           |
| **Mutability**        | Mutable by default            | Immutable by default, explicit `mut`    |
| **Type System**       | Structural, optional          | Nominal, mandatory                      |
| **Concurrency**       | Single-threaded (event loop)  | Multi-threaded, fearless                |
| **Runtime**           | V8, Node.js, Deno             | No runtime, compiled to native          |
| **Package Manager**   | npm, yarn, pnpm               | Cargo (built-in)                        |
| **Interfaces**        | interface, type               | trait                                   |
| **Async**             | Promise, async/await          | Future, async/await (similar syntax!)   |
| **Runtime Speed**     | Good (JIT)                    | Excellent (AOT, no GC pauses)           |
...
All 31 chapters (00–30) are complete and code-verified. Every Rust example compiles on current stable Rust (1.96.0, 2024 edition); the six capstone crates build natively (and `cargo check` for `wasm32` where relevant). Intentionally non-compiling snippets that illustrate pitfalls are clearly marked.
