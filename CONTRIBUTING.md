...
`npm test` covers the Cloudflare Worker's redirects and Markdown content
negotiation. The production build also validates every internal link and anchor.

#### Test the Rust Capstone Projects

The repository pins Rust, rustfmt, Clippy, and the WebAssembly target in
`rust-toolchain.toml`. Run the same gates as CI from the repository root:
...
cargo check --manifest-path examples/wasm-app-code/Cargo.toml \
  --target wasm32-unknown-unknown --locked
cargo check --manifest-path examples/full-stack-code/Cargo.toml \
  --package frontend --target wasm32-unknown-unknown --locked
```

#### Check Page Snippets

Run the automated standard-library snippet gate:

```bash
npm run test:snippets
```

It extracts self-contained `rust playground` programs from normal lesson
sections and compiles them with the repository's pinned Rust 2024 toolchain.
Fragments and Common Pitfalls/Exercises sections are excluded. For new or changed snippets that need
external crates, a database, the network, or multiple files, verify them in a
temporary Cargo project and state the required dependencies beside the example.
Deliberately failing examples must be identified as such and include the
expected compiler error. The [version and verification policy](./src/content/docs/00-introduction/05-version-policy.md)
defines exactly what each CI gate proves.
