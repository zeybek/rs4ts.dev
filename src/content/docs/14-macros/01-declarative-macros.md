...
A declarative macro lets you define new *syntax* that expands into code. Below, `hashmap!` accepts `key => value` pairs and expands — at compile time — into a sequence of `insert` calls. The repository's [pinned verification toolchain](/00-introduction/05-version-policy/) uses the 2024 edition; `cargo new` selects that edition automatically, and `macro_rules!` works on every edition.
