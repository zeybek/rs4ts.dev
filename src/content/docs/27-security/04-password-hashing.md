...
- **Never manage salts manually.** Let `SaltString::generate(&mut OsRng)` and the PHC string handle it. `rand::rng()` / `ThreadRng` is also a CSPRNG, but the password-hash API's explicit `OsRng` path is clearer and avoids fork-state caveats. The real bug is using a non-CSPRNG such as `SmallRng`, a timestamp, or a fixed seed — see [Secure Randomness](/27-security/06-secure-randomness/).
...
> **Note:** This page presents `argon2 = "0.5"`, `bcrypt = "0.19"`, and `thiserror = "2"`. The repository's [pinned verification toolchain](/00-introduction/05-version-policy/) uses the 2024 edition, which `cargo new` selects automatically. Always run `cargo add <crate>` to resolve the latest compatible versions rather than copying pins.
