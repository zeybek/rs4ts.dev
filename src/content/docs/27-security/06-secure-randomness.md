...
`ThreadRng` does not automatically reseed after a process fork. Code that calls `fork()` must call `rand::rng().reseed()` immediately in the child, or use `SysRng` for secrets generated around that boundary. Most Rust services never fork after startup, but crypto guidance should state the threat-model exception explicitly.
