...
> **Note:** The examples use SQLx 0.8 with the bundled SQLite driver so they run with zero setup (`sqlite::memory:`). The same `bind`/placeholder API applies to PostgreSQL and MySQL; only the placeholder syntax differs (`$1` for Postgres, `?` for SQLite/MySQL). The repository's [pinned verification toolchain](/00-introduction/05-version-policy/) uses the 2024 edition; `cargo new` selects that edition automatically.
