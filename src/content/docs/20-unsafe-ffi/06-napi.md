...
- **Return `napi::Result<T>` and build errors with `Error::new(Status::…, msg)`** so failures surface as proper JS exceptions with a `.code`, instead of relying on `unwrap()`/`panic!`. Framework and panic-profile behavior differs: a panic may be caught and converted, or it may abort the process at a non-unwinding ABI boundary; neither is a stable error contract for JavaScript callers.
