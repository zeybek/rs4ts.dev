...
> **Note:** The repository's [pinned verification toolchain](/00-introduction/05-version-policy/) uses the 2024 edition; `cargo new` selects that edition automatically. Several behaviors on this page (`static mut` references being a hard error, `unsafe extern` blocks, and `unsafe fn` bodies no longer being implicitly unsafe) are 2024-edition defaults. Where they differ from older Rust, this page says so.
