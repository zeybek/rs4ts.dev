...
Neon 1.x offers two coexisting styles, and idiomatic code uses both. The high-level `#[neon::export]` attribute auto-converts plain Rust types to and from JavaScript; the low-level `FunctionContext` style hands you raw JavaScript handles to parse yourself. This page targets the repository's [pinned verification toolchain](/00-introduction/05-version-policy/) and the 2024 edition.
