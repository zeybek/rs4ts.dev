...
> **Note:** The repository's [pinned verification toolchain](/00-introduction/05-version-policy/) uses the 2024 edition; `cargo new` selects that edition automatically. Two edition-2024 details matter for FFI: `#[no_mangle]` must now be written `#[unsafe(no_mangle)]`, and `extern` blocks must be written `unsafe extern`. Both are explained below.
...
> **Warning:** A plain `extern "C"` boundary is non-unwinding. On current Rust, a Rust panic that reaches that boundary aborts the process instead of unwinding into C. A native foreign exception crossing the same non-unwinding boundary is undefined behavior. Catch Rust panics when you need to translate them into an error code (shown in the Real-World Example), or use the `"C-unwind"` ABI only when both sides intentionally support unwinding.
...
### Pitfall 4: Letting a panic reach a non-unwinding boundary

A `panic!` (or an arithmetic overflow in debug, an `unwrap` on `None`, an out-of-bounds index) that reaches an `extern "C"` boundary does not turn into a C error or exception. With Rust's unwinding panic runtime, the non-unwinding ABI boundary converts it to a process abort. If the API must report failure without terminating the process, catch an unwinding Rust panic with `std::panic::catch_unwind` and return an error code instead (next section). `catch_unwind` cannot catch a build configured with `panic = "abort"`.
...
- **Define the panic policy at the boundary.** Wrap an `extern "C"` body in `std::panic::catch_unwind` when the contract requires a return code instead of process termination. If the library uses `panic = "abort"`, document that any panic terminates the process; `catch_unwind` cannot recover it.
...
pub extern "C" fn checked_add(a: c_int, b: c_int, out: *mut c_int) -> Status {
    // Convert an unwinding Rust panic into the C-facing status code.
...
The first call adds `2_000_000_000 + 2_000_000_000`, which overflows a 32-bit `int`, so `checked_add` returns `Status::Overflow` (status `1`) and leaves `out` untouched. The second call succeeds with status `0` and `out = 42`. With the normal unwinding panic runtime, panics raised inside the closure become `Status::Panicked` instead of aborting at the ABI boundary. The caller must still honor the pointer contract, and a `panic = "abort"` build still terminates rather than returning a status.
