...
We will write a tiny C "text" library and call it from Rust. The repository's [pinned verification toolchain](/00-introduction/05-version-policy/) uses the 2024 edition, and `cargo new` selects that edition automatically. Start a binary crate and add `cc` as a **build dependency**:
