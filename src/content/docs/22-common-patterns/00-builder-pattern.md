...
In production you rarely hand-roll the boilerplate. The `bon` crate gives you a derive macro that produces a builder *and* makes missing required fields a compile error: no `Result`, no `unwrap`, no manual `Option` plumbing. With the repository's [pinned verification toolchain](/00-introduction/05-version-policy/), the recorded dependency resolution was bon 3.9.1:
