...
description: "Rust has no exceptions or null: a fallible function returns Result<T, E> and a maybe-empty one returns Option<T>, keeping failure and absence visible in the type."
---

Rust has no exceptions and no `null`. Instead, a function that can fail returns a **`Result<T, E>`** value, and a function that might have nothing to return uses **`Option<T>`**. Both are ordinary enums, so the possibility of failure or absence is written into the type. To obtain the inner value, you must match, propagate, choose a fallback, or deliberately panic.
...
Because these are return values, not control-flow jumps, **the type signature tells you a function can fail**. Discarding an entire `Result` is still possible, but its `#[must_use]` annotation emits an `unused_must_use` warning by default; many projects promote that warning to an error in CI.
...
| Can you ignore it?      | A thrown error is invisible in the signature   | Discarding is explicit or warns; extracting `T` requires choosing a path |
