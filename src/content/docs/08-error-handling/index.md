...
- Why Rust replaces `throw`/`try`/`catch` with the **`Result<T, E>`** return value and `null`/`undefined` with **`Option<T>`**. Failure and absence become part of the type; extracting the success value requires an explicit choice, while discarding the whole `Result` emits an `unused_must_use` warning by default
