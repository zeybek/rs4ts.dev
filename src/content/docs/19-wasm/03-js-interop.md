...
Create a library crate configured to build a `cdylib` (covered in [wasm-pack](/19-wasm/01-wasm-pack/)), then add the two crates that power JavaScript interop. The current stable toolchain is Rust 1.96.0 on the 2024 edition, and `cargo new` selects that edition automatically:
