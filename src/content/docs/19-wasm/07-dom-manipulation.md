...
Create a library crate that builds a `cdylib` (see [Setting Up wasm-pack](/19-wasm/01-wasm-pack/)), then add the boundary crates. The repository's [pinned verification toolchain](/00-introduction/05-version-policy/) uses the 2024 edition, and `cargo new` selects that edition automatically:
