...
This page walks the full round trip: write the Rust, run one build command, and wire the result into an HTML page. The repository's [pinned verification toolchain](/00-introduction/05-version-policy/) uses the 2024 edition, and `cargo new` selects that edition automatically. The examples here use **wasm-bindgen 0.2** and **wasm-pack 0.13**.
