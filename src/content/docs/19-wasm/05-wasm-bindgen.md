...
Create a library crate (WASM modules are libraries, not binaries) and add the boundary crates. The repository's [pinned verification toolchain](/00-introduction/05-version-policy/) uses the 2024 edition; `cargo new` selects that edition automatically.
