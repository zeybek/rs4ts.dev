...
The repository's [pinned verification toolchain](/00-introduction/05-version-policy/) uses the 2024 edition; `cargo new` selects that edition automatically. The recorded dependency set is wasm-bindgen 0.2.122, web-sys/js-sys 0.3.99, wasm-bindgen-futures 0.4.72, serde-wasm-bindgen 0.6, and wasm-pack 0.13; complete projects are checked against `wasm32-unknown-unknown` as described by the verification policy.
