...
> **Note:** This file targets `indicatif` **0.18**, the repository's [pinned Rust toolchain](/00-introduction/05-version-policy/), and the 2024 edition. The 0.18 API centers on `ProgressBar`, `ProgressStyle::with_template`, and `MultiProgress`. The pre-0.17 `ProgressStyle::default_bar().template("…")` chaining and the old `{wide_bar}`-only styling are superseded by the `with_template(...).unwrap()` builder shown throughout.
