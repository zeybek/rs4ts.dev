...
> **Note:** Every Rust snippet here was compiled and run with `cargo` 1.96.0 (the recorded verification toolchain; 2024 edition). The SQLx examples were verified against SQLx **0.8.6**; the `Pool`/`PoolOptions` API shown is identical in 0.8 and the newer 0.9 line that `cargo add sqlx` now resolves to on Rust ≥ 1.94. The generic-pooler examples use **`deadpool` 0.14** and **`bb8` 0.9**. SQLx examples run against in-memory SQLite, so they reproduce with no server installed.
