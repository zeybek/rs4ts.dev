...
> **Note:** The repository's [pinned verification toolchain](/00-introduction/05-version-policy/) uses the 2024 edition; `cargo new` selects that edition automatically. `GlobalAlloc`, `#[global_allocator]`, and `std::alloc::System` are all long-stable — none of this needs nightly.
...
You may have seen `Vec::new_in(my_alloc)` and the `Allocator` trait. On the repository's [pinned Rust 1.96.0 baseline](/00-introduction/05-version-policy/), that per-collection allocator API (`allocator_api`) is nightly-only. On that stable baseline you choose the allocator *once*, globally, via `#[global_allocator]`. If you need region/arena allocation for a subset of your data, reach for a crate like `bumpalo` (which gives you `bumpalo::Bump` and its own `Vec`/`String` types) unless the current stable documentation confirms `allocator_api` has stabilized.
