...
A trait is a set of methods a type promises to provide, like an interface. The differences: you can implement a trait for a type you do not own, traits can ship default method bodies, and the compiler usually dispatches them statically with zero runtime cost.
...
Yes, through **WebAssembly**. You compile a Rust crate to a `.wasm` module and call it from JavaScript like any module, which is how teams move hot paths (parsing, image work, crypto) off the main thread. `wasm-bindgen` generates the glue and the TypeScript types.
