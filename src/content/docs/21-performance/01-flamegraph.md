...
> **Note:** `cargo install flamegraph` gives you the `cargo flamegraph` subcommand. The example uses the repository's [pinned verification toolchain](/00-introduction/05-version-policy/) and the 2024 edition. On Linux `cargo-flamegraph` drives `perf`; on macOS it drives `dtrace` (which ships with the OS, but the profiler needs elevated privileges, so you run it under `sudo`). See [Profiling Rust Applications](/21-performance/00-profiling/) for the platform setup.
