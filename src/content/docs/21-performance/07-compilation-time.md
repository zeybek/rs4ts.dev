...
The repository's [pinned verification toolchain](/00-introduction/05-version-policy/) uses the 2024 edition; `cargo new` selects that edition automatically, and `resolver = "3"` is the edition-2024 feature resolver. Running `cargo run -p app` from the workspace root prints:
