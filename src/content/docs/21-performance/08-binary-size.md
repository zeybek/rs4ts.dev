...
Start from a small but realistic program. Create it with `cargo new word_freq` (which selects the 2024 edition on a modern toolchain), then put this in `src/main.rs`:
...
A plain `cargo build --release` produces a binary of **462,992 bytes** (~452 KB on this machine, Rust 1.96.0, the recorded toolchain, on macOS). Now add a size-tuned release profile:
