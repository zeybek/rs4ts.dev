...
> **Note:** This page uses **axum 0.8** and **tower-http 0.6**. The repository's [pinned verification toolchain](/00-introduction/05-version-policy/) uses the 2024 edition; `cargo new` selects that edition automatically. Servers start with `axum::serve(listener, app)` and a `tokio::net::TcpListener`.
