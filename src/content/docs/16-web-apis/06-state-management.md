...
> **Note:** This page targets the **axum 0.8** API line (recorded with 0.8.9) and the Rust 2024 edition. State is attached with `Router::with_state(...)` and read with the `State<T>` extractor. There is no `Server::bind()` builder and path parameters use `{id}`, not `:id`. The `#[derive(FromRef)]` macro shown later needs `cargo add axum --features macros`.
