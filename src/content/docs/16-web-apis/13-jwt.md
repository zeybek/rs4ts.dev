...
The repository's [pinned verification toolchain](/00-introduction/05-version-policy/) uses the 2024 edition; `cargo new` selects that edition automatically. This page targets **axum 0.8** and **`jsonwebtoken` 10**.
...
if (!SECRET || Buffer.byteLength(SECRET, "utf8") < 32) {
  throw new Error("JWT_SECRET must contain at least 32 unpredictable bytes");
}
...
- **Load the secret from the environment, never hard-code it or provide a fallback.** `std::env::var("JWT_SECRET")` must fail startup when the variable is missing. For HS256, require at least 32 unpredictable bytes; a weak or leaked HMAC secret means anyone can forge tokens. See [Deployment](/16-web-apis/19-deployment/) for env config.
...
A production-structured auth module: a `Keys` bundle in shared state, a `Claims` struct with a typed `Role`, a custom `AuthError` that implements `IntoResponse`, an `AuthUser` extractor for "any logged-in user", and an `AdminUser` extractor that layers a role check on top. The credential check remains deliberately hard-coded so the example stays focused on JWT mechanics; replace it with an Argon2-backed user lookup before deploying. Add the crates first:
...
#[tokio::main]
async fn main() {
    // Fail closed: starting with a public fallback key would let anyone forge tokens.
    let secret = std::env::var("JWT_SECRET")
        .expect("JWT_SECRET must be set to at least 32 unpredictable bytes");
    assert!(
...
async fn main() {
    let secret = std::env::var("JWT_SECRET")
        .expect("JWT_SECRET must be set to at least 32 unpredictable bytes");
    assert!(secret.as_bytes().len() >= 32, "JWT_SECRET is too short");
    let state = AppState {
