...
This is the Rust counterpart to `order?.customer?.address?.zip` — but `?` works on owned and borrowed `Option`s alike, and the compiler guarantees you handled every `None`.
