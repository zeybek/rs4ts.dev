...
JavaScript's single `f64` loses integer precision past 2^53 and hides cost. Rust's sized integers (`i32`, `u64`, `usize`, …) and floats (`f32`, `f64`) make range, signedness, and memory explicit. Constant overflow is rejected, debug integer overflow panics by default, and optimized release builds wrap unless overflow checks are enabled; use `checked_*`, `saturating_*`, or `wrapping_*` when the policy matters. See [Basic Types](/02-basics/01-types/).
