...
> **Note:** All examples here target the repository's [pinned verification toolchain](/00-introduction/05-version-policy/) and the 2024 edition; `cargo new` selects that edition automatically. `Box` is the heap-allocating smart pointer covered in [Section 10: Smart Pointers](/10-smart-pointers/).
