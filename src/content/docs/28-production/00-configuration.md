...
> The repository's [pinned verification toolchain](/00-introduction/05-version-policy/) uses the 2024 edition; `cargo new` selects that edition automatically, and `cargo add` (built into Cargo since 1.62, no `cargo-edit` needed) resolves the newest compatible versions. The examples here use `config` 0.15, `serde` 1, and `figment` 0.10.
