...
First, the dependencies. The repository's [pinned verification toolchain](/00-introduction/05-version-policy/) uses the 2024 edition; `cargo new` selects that edition automatically. Add `serde_json` (and `serde` if you also want typed conversion):
