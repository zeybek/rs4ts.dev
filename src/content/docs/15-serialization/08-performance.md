...
The Rust version borrows the `metric` string straight out of the line buffer (no per-record string allocation), reuses one `String` for reading lines, and never builds an intermediate dynamic tree. The repository's [pinned verification toolchain](/00-introduction/05-version-policy/) uses the 2024 edition; `cargo new` selects that edition automatically.
