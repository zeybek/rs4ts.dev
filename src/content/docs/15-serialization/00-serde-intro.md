...
> **Note:** The repository's [pinned verification toolchain](/00-introduction/05-version-policy/) uses the 2024 edition; `cargo new` selects the newest edition automatically. `cargo add serde --features derive` and `cargo add serde_json` add the dependencies above. `cargo add` has been built into Cargo since 1.62, so no extra tooling is required.
