...
- **Use a current compatible Tokio 1.x release.** This guide records the dependency version beside each complete example and checks against the repository's [pinned Rust toolchain](/00-introduction/05-version-policy/) on the 2024 edition. Avoid copying pre-1.0 or pre-`axum 0.8` tutorials whose runtime/server APIs have since changed.
