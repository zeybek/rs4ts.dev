...
> **Note:** `Stream` is **not** in the standard library's prelude the way `Iterator` is. On the repository's [pinned Rust 1.96.0 baseline](/00-introduction/05-version-policy/), `std::async_iter::AsyncIterator` is nightly-only and unstable; production code uses the `Stream` trait from the [`futures`](https://docs.rs/futures) crate, re-exported by [`tokio-stream`](https://docs.rs/tokio-stream). The recorded dependency set is Tokio 1.52, `tokio-stream` 0.1, `futures` 0.3, and `async-stream` 0.3 on the 2024 edition.
