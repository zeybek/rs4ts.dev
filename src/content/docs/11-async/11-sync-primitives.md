...
> **Note:** Every Rust snippet on this page was compiled and run with `cargo`/`rustc` 1.96.0 (the recorded verification toolchain; 2024 edition). Async examples use `tokio = { version = "1.52", features = ["full"] }`. Rust ships **no built-in async runtime**, so the runtime is always explicit. See [Tokio Setup](/11-async/03-tokio-setup/).
