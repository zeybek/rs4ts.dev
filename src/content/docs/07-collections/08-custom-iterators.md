...
TypeScript developers often look for a `yield` keyword. On the repository's [pinned Rust 1.96.0 baseline](/00-introduction/05-version-policy/), stable Rust has **no generator syntax**: you cannot write `gen { yield x; }` in a normal function. You must encode the state by hand in a struct, as shown throughout this page. (An experimental `gen` block exists on nightly in that baseline; check the current tracking status before relying on it.) For the common case of "I just want to transform an existing collection," you usually do *not* need a custom iterator at all. Chain adaptors on the collection's built-in iterator instead (see [Iterators](/07-collections/06-iterators/)).
